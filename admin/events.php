...
if($_REQUEST['del_id']>0){
	...
	mysql_query("delete from events where id='$del_id'");
}
if($_REQUEST['stat_id']>0){
	...
	mysql_query("update events set active='$active_st' where id='$stat_id'");
}
?>
...
<form controller="referFriends" id="ReferFriendIndexForm" method="post" action="events.php" accept-charset="utf-8">	
...
<table cellpadding="0" cellspacing="0" class="index_table centered_table all_borders">
	...
	<?php
		...
		$sql_get="select * from events where id>0 $title_whr";
		$result_get=mysql_query($sql_get);
		$link_nam="events.php?p=";
		$recordsPerPage = 30;
	...
	<tr>
		...
		<td class="font_10_black"><?php echo $row['event_date'];?></td>
		<td class="font_10_black">
		 <a  href='events.php?stat_id=<?php echo $row['id']; ?>&active=<?php echo $row['active']; ?>'>
			<?php if($row['active']=="1"){
			?>
		...
		<td class="font_10_black">
			<a href="events_edit.php?action=edit&edit_id=<?php echo $row['id']; ?>"><img src="images/edit_link.png" alt="" /></a>			
		</td>
		<td class="font_10_black">
		 <a  href='events.php?action=del&del_id=<?php echo $row['id']; ?>' onClick="return checkdel();"> <img src="images/deleteIcon.png" border="0" alt="delete"> </a>	
		</td>
	</tr>
	<?php }} ?>
</td>
