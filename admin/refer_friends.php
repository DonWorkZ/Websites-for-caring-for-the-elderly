...
if($_REQUEST['del_id']>0){
	...
	mysql_query("delete from refer_friends where id='$del_id'");
}
?>
...
<tr>
	<td class="font_11_black">
		The refer a friend  has been saved.
	</td>
</tr>
<?php } ?>
<tr>
...
<table cellspacing='0px' cellpadding='0px' id="content_heading_table">
	<tr>
		<td class="content_heading">
			Refer A Friend Manager		
		</td>
	</tr>
</table>	
<form controller="referFriends" id="ReferFriendIndexForm" method="post" action="refer_friends.php" accept-charset="utf-8">	
...
<table cellpadding="0" cellspacing="0" class="index_table centered_table all_borders">
	...
	<?php
		...
		$sql_get="select * from refer_friends where id>0 $fname_whr $lname_whr $email_whr";
		$result_get=mysql_query($sql_get);
		$link_nam="refer_friends.php?p=";
		$recordsPerPage = 30;
	...
	<tr>
		...
		<a href="refer_friends_edit.php?action=edit&edit_id=<?php echo $row['id']; ?>"><img src="images/edit_link.png" alt="" /></a>			
		</td>
		<td class="font_10_black">
		 <a  href='refer_friends.php?action=del&del_id=<?php echo $row['id']; ?>' onClick="return checkdel();"> <img src="images/deleteIcon.png" border="0" alt="delete"> </a>	
		</td>
	</tr>
	<?php }} ?>
	</td>
