 ...
 */
	function day($fieldName, $selected = null, $attributes = array()) {
		...
		$defaults = array('twoDigitDay' => false);
		$attributes = array_merge($defaults, (array) $attributes);
		$twoDigitDay = $attributes['twoDigitDay'];

		return $this->select($fieldName . ".day", $this->__generateOptions('day', array('twoDigitDay' => $twoDigitDay)), $selected, $attributes);
	}
	...
	function __generateOptions($name, $options = array()) {
		...
		switch ($name) {
			...
			case 'day':
				...
				$dayFormat = "%d";
				if ($options['twoDigitDay'] === true) {
					$dayFormat = '%02d';
				}

				for ($i = $min; $i <= $max; $i++) {
					$data[sprintf('%02d', $i)] = sprintf($dayFormat, $i);
				}
			break;
			case 'month':
